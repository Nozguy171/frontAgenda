// frontend/src/components/RejectedSection.tsx
'use client'
...
export default function RejectedSection() {
  ...
  useEffect(() => {
    apiFetch('/prospects').then(all => {
      setList(all.filter((p: any) => p.status === 'rejected'))
      setLoading(false)
    })
  }, [])

  const handleRecover = async (id: number) => {
    await apiFetch(`/prospects/${id}/status?status=pending`, { method: 'PATCH' })
  ...
  const handleCall = async (id: number) => {
    ...
    // opcional: refrescar o quitar de lista
    setList(list.filter(p => p.id !== id))
  }
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
      {list.length ? list.map(p => {
        ...
        <p className="text-gray-400 italic">No hay prospectos rechazados.</p>
      )}
    </div>
  )
}
